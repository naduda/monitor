...
(() => {
		var ctrls = security.controllers,
				dirs = security.directives,
				serv = security.services;
		var app = angular.module('Security', ['ngRoute', 'ngAnimate']);

		app.run((authService, $rootScope, $route, $timeout,
				translate: security.services.TranslateService,
				dataService: security.services.DataService) => {
